...
4. Если `ym` ещё нет — создаётся тег `<script src="https://mc.yandex.ru/metrika/tag.js" async>`, вставляется в `document.head`. После события `onload` вызываются `ym(id, "init", defaultParams)` и хит для текущего URL.
5. В `useEffect` с пустым массивом зависимостей при размонтировании скрипт удаляется из DOM (cleanup).
