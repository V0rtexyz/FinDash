  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
        ...
        if (registerResponse.success) {
          ...
          try {
            ...
            if (
              ...
              if (token && userStr) {
                try {
                  ...
                  setSuccess("");
                  setError(
                    "Ошибка обработки данных пользователя. Попробуйте войти вручную."
                  );
                }
              } else {
                setSuccess("");
                setError(
                  "Ошибка сохранения данных авторизации. Попробуйте войти вручную."
                );
              }
            } else {
              setSuccess("");
            ...
            if (
              ...
              errorMessage.includes("база данных недоступна")
            ) {
              setError(
                "Регистрация успешна, но не удалось войти из-за проблем с базой данных. Попробуйте войти вручную."
              );
            } else if (errorMessage.includes("Неверный логин или пароль")) {
              setError(
                "Регистрация успешна, но автоматический вход не удался. Попробуйте войти вручную через несколько секунд."
              );
            } else {
              setError(
                `Регистрация успешна! ${errorMessage}. Попробуйте войти вручную.`
              );
            }
          }
        } else {
      ...
      if (
        ...
        setError(
          "База данных недоступна. Приложение работает в режиме без сохранения данных. Попробуйте позже или настройте подключение к PostgreSQL."
        );
      } else if (
        ...
        errorMessage.includes("Registration error") ||
        errorMessage.includes("Ошибка регистрации")
      ) {
        setError(
          "Ошибка при регистрации. Попробуйте еще раз или войдите, если уже зарегистрированы."
        );
      } else {
